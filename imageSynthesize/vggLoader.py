...
def image_from_vgg_converter(x):
    '''Preconditioning an image from vgg model'''
    x = x.transpose((1, 2, 0))
    x[:, :, 0] += 103.939
    x[:, :, 1] += 116.779
    x[:, :, 2] += 123.68
...
def get_model(img_width, img_height, weights_path='vggLoader_weights.h5', pool_mode='avg'):
