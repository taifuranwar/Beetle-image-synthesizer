...
def find_synthesize_patches(a, a_prime, b, patch_size=3, patch_stride=1):
    '''This is for precalculating the synthesize_loss
...
def synthesize_loss(a, a_prime, b, b_prime, patch_size=3, patch_stride=1, use_full_synthesize=False):
    '''http://www.mrl.nyu.edu/projects/image-Synthesize/index.html'''
    best_a_prime_patches = find_synthesize_patches(a, a_prime, b, patch_size=patch_size, patch_stride=patch_stride)
    if use_full_synthesize:  # combine all the patches into a single image
        ...
        bs = b.shape
        b_synthesize = patches.combine_patches(best_a_prime_patches, (bs[1], bs[2], bs[0]))
        loss = content_loss(np.expand_dims(b_synthesize, 0), b_prime)
    return loss
